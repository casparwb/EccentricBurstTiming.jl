module EccentricBurstTiming
    ...
    mutable struct BurstTimingModel{T, vecT}
        ...
        function BurstTimingModel(;e0 = 0.99, p0 = 30, t0 = 0, M = 60, eta = 0.20,
                                   w0 = 0, m3 = 1e7, R3 = 1.1e7, V3 = π/3,
                                   lambda = 0, iota = 0)
            T1 = typeof(e0)
            T2 = typeof(T1[])

            return new{T1, T2}(lambda, iota, M, eta, m3 / M, m3, R3, 
    ...
    function get_t_next(model)
        ...
        two_body_update = 2π/√model.M*((model.pprev + model.eta*(model.M^(5/2)/model.pprev^(3/2))*Acoeff) / 
                                       (1 - model.eprev^2 + model.eta*(model.M/model.pprev)^(5/2)*Bcoeff))^(3/2)
    ...
    function main(; e0 = 0.99, p0 = 30, t0 = 0, M = 60, eta = 0.20,
                    w0 = 0,m3 = 1e7,R3 = 1.1e7, V3 = π/3,
                    N = 73, A = 1e-21, geocent_time = 0, 
                    ra = 0, dec = 0, psi = 0, lambda = 0,iota = 0)

        burst_model_perturbed = BurstTimingModel(e0=e0, p0=p0, t0=0,  w0=w0, M=1, eta=eta,
                                                m3=m3, R3=R3, V3=V30, lambda=lambda, iota=iota)
        evolve!(burst_model_perturbed, N_request)

        burst_model_perturbed_tilted = BurstTimingModel(e0=e0, p0=p0, t0=0,  w0=w0, M=1, eta=eta,
                                                m3=m3, R3=R3, V3=V30, lambda=np.pi/6, iota=np.pi/4)
        evolve!(burst_model_perturbed_tilted, N_request)

        burst_model_unperturbed = BurstTimingModel(e0=e0, p0=p0, t0=0, w0=w0, M=1, eta=eta,
        ...
        p_array_perturbed = burst_model_perturbed.p[end-N:end]*M*Constants.Mconvert/1e-5 # [au / 10^-5]
        ...
        p_array_perturbed_tilted = burst_model_perturbed_tilted.p[end-N:end]*M*Constants.Mconvert/1e-5 # [au / 10^-5]
        ...
        p_array_unperturbed = burst_model_unperturbed.p[end-N:end]*M*Constants.Mconvert/1e-5 # [au / 10^-5]
        ...
        burst_signal_unperturbed = Utils.burst_signal(t, t_array_unperturbed, A, M)
    end
